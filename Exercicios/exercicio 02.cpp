#include <iostream>
using namespace std;
/* Desenvolva uma aplicação para cadastrar cliente que fará um crédito em 12 vezes. */
/* Insira na estrutura informações pertinentes ao cliente. */


struct cadastro{
	string Nome;
	float Compra;
	string Loja;
};

string Nome_cliente();
float Compra_realizada();
string Nome_loja();

int main() {
	
	struct cadastro vetor_cliente;
	
	vetor_cliente.Nome = Nome_cliente();
	vetor_cliente.Loja = Nome_loja();
	vetor_cliente.Compra = Compra_realizada();
	
	cout << "Nome do cliente: " << vetor_cliente.Nome << endl;
	cout << "Nome da loja: " << vetor_cliente.Loja << endl;
	cout << "Valor da Parcela: " << vetor_cliente.Compra << endl;
	return 0;
}

string Nome_cliente(){
	string nome;
	cout << "Insira o nome do cliente: " << endl;
	cin >> nome;
	return nome;
}

float Compra_realizada(){
	float compra;
	float result;
	cout << "Insira o valor da compra: " << endl;
	cin >> compra;
	result = compra / 12;
	return result;
}

string Nome_loja(){
	string loja;
	cout << "Insira o nome da loja: " << endl;
	cin >> loja;
	return loja;
}